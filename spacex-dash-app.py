...
def get_scatter_chart(selected_site, payload_range):
    ...
    if selected_site == 'All':
        fig = px.scatter(df,x='Payload Mass (kg)', y = 'class', color = 'Booster Version Category', title = 'Payload vs Outcome for All Sites')
        return fig
    else:
        df_site = df[df['Launch Site'] == selected_site]
        fig = px.scatter(df_site, x = 'Payload Mass (kg)', y = 'class', color = 'Booster Version Category', title = f'Payload vs Outcome for {selected_site}')
        return fig
# Run the app
